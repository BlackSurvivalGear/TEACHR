...
assert(chat.includes('includeCurriculum: false'), 'Stage 2 chat must not silently add curriculum context before Stage 3');
...
assert(transport.includes("includeCurriculum = tool !== 'chat'"), 'chat must default to no curriculum context until Stage 3');
assert(apps.includes("const isChat = body.tool === 'chat'"), 'Apps Script must recognise the dedicated chat route');
assert(apps.includes('generationProfile_(uid)'), 'chat must still require an active TEACHR member profile');
assert(apps.includes('const usage = isChat ? null'), 'Stage 2 chat must not consume the six generator allowances');
