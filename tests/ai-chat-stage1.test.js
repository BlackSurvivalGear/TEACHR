...
assert(/event\.key\s*===\s*'Enter'\s*&&\s*!event\.shiftKey/.test(js), 'Enter should submit while Shift+Enter remains available');
assert(/event\.key\s*===\s*'Escape'/.test(js), 'Escape should close the panel');
...
assert(css.includes('html:not([data-auth="signed-in"]) .teachr-chat-launcher'), 'Ask TEACHR must be hidden on the public signed-out page');
