...
function generationError_(status, code, message) { const error = new Error(message); error.status = status; error.code = code; return error; }
function doPost(e) { let result; try { const raw=e&&e.postData&&e.postData.contents; if(typeof raw!=='string'||raw.length>100000) throw generationError_(400,'INVALID_REQUEST','Invalid generation request.'); let body; try{body=JSON.parse(raw);}catch(_){throw generationError_(400,'INVALID_JSON','Invalid JSON.');} if(!body||body.action!=='generate') throw generationError_(400,'INVALID_ACTION','Unsupported action.'); result=generateResource_(body); } catch(error){ result={ok:false,status:error.status||503,code:error.code||'GENERATION_UNAVAILABLE',error:error.code?error.message:'Generation service is unavailable. Please try again.'}; } return ContentService.createTextOutput(JSON.stringify(result)).setMimeType(ContentService.MimeType.JSON); }
function verifyGenerationIdentity_(idToken) { if(typeof idToken!=='string'||!idToken||idToken.length>10000) throw generationError_(401,'AUTH_REQUIRED','Sign in to generate resources.'); const response=UrlFetchApp.fetch('https://identitytoolkit.googleapis.com/v1/accounts:lookup?key=AIzaSyAF4zqPPPqoYA1hwNyorfqinsQkglNp6lQ',{method:'post',contentType:'application/json',payload:JSON.stringify({idToken}),muteHttpExceptions:true}); if(response.getResponseCode()!==200) throw generationError_(401,'INVALID_TOKEN','Your sign-in session is invalid or expired.'); const user=JSON.parse(response.getContentText()).users?.[0]; if(!user||!user.localId||user.disabled) throw generationError_(401,'INVALID_TOKEN','Your sign-in session is invalid or expired.'); if(!user.emailVerified) throw generationError_(403,'EMAIL_NOT_VERIFIED','Verify your email address before generating resources.'); return user.localId; }
