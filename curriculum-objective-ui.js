(() => {
  ...
  style.textContent = `
    ...
    .curriculum-alignment[hidden]{display:none}.curriculum-alignment-head{display:flex;justify-content:space-between;align-items:start;gap:16px;margin-bottom:12px}
    .curriculum-alignment-label{display:block;color:#70bfff;font-size:9px;font-weight:800;letter-spacing:.12em;margin-bottom:4px}.curriculum-alignment-head strong{display:block;color:#eaf3ff;font-size:13px}
  ...
  const YEAR_TO_KEY_STAGE = {'Year 1':'KS1','Year 2':'KS1','Year 3':'KS2','Year 4':'KS2','Year 5':'KS2','Year 6':'KS2','Year 7':'KS3','Year 8':'KS3','Year 9':'KS3','Year 10':'KS4','Year 11':'KS4'};
  let autoObjective = '';
  let selectionKey = '';
  ...
  year.closest('.field')?.appendChild(stageLabel);
  ...
  const keyStage = () => YEAR_TO_KEY_STAGE[year.value] || '';
  const isEngland = () => /England\s*[—-]\s*National Curriculum/i.test(curriculum.value || '');
  const currentTopic = () => topic.value === '__custom__' ? (document.getElementById('customTopic')?.value || '') : topic.value;
  const currentKey = () => [curriculum.value, subject.value, year.value, currentTopic()].join('|');

  function dispatchObjectiveInput() { objective.dispatchEvent(new Event('input', { bubbles: true })); }
  function clearAutoObjective() {
    if (autoObjective && objective.value.trim() === autoObjective) {
      objective.value = '';
      dispatchObjectiveInput();
    }
    autoObjective = '';
  }
  function setAutoObjective(value) {
  ...
  function clearAlignment(message = 'Select a subject and topic') {
    title.textContent = message; status.textContent = 'Not resolved'; list.innerHTML = ''; alignment.hidden = !isEngland();
  }
  ...
  async function resolveCurriculum() {
    ...
    try {
      ...
      const resolved = registry.resolve({curriculum:curriculum.value,subject:subject.value,year:year.value,topic:currentTopic()});
      if (currentKey() !== nextKey) return;
      renderAlignment(resolved);
    } catch { clearAlignment('Curriculum alignment could not be resolved'); }
  }

  objective.addEventListener('input', () => {
    if (autoObjective && objective.value.trim() !== autoObjective) autoObjective = '';
  });
  [curriculum, subject, year, topic].forEach(input => { input.addEventListener('change', resolveCurriculum); input.addEventListener('input', resolveCurriculum); });
  form.addEventListener('submit', () => renderKeyStage(), true);
  renderKeyStage(); resolveCurriculum();
})();
