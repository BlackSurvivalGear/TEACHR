...
function updateStats() { const resources = getResources(); els.statResources.textContent = resources.length; els.statLessons.textContent = resources.filter(r => r.type === 'LESSON PLAN').length; els.statAssessments.textContent = resources.filter(r => r.type === 'ASSESSMENT').length; if (els.heroResourceCount) els.heroResourceCount.textContent = resources.length; }
function updateHeroTopic() { if (els.heroTopic) els.heroTopic.textContent = `${document.getElementById('year').value || 'Year 8'} · ${document.getElementById('topic').value || 'Fractions'}`; }
