// Server-only generation. Keep Code.gs checkout handlers unchanged.
function generationError_(status, code, message) {
...
function doPost(e) {
  ...
  try {
    ...
    if (typeof raw !== 'string' || raw.length > 100000) throw generationError_(400, 'INVALID_REQUEST', 'Invalid generation request.');
    let body;
    try { body = JSON.parse(raw); } catch (_) { throw generationError_(400, 'INVALID_JSON', 'Invalid JSON.'); }
    if (!body || body.action !== 'generate') throw generationError_(400, 'INVALID_ACTION', 'Unsupported action.');
    result = generateResource_(body);
  } catch (error) {
    result = { ok: false, status: error.status || 503, code: error.code || 'GENERATION_UNAVAILABLE', error: error.code ? error.message : 'Generation service is unavailable. Please try again.' };
  }
  // ContentService cannot set HTTP error status: clients must check this envelope.
  return ContentService.createTextOutput(JSON.stringify(result)).setMimeType(ContentService.MimeType.JSON);
}
function verifyGenerationIdentity_(idToken) {
  if (typeof idToken !== 'string' || !idToken || idToken.length > 10000) throw generationError_(401, 'AUTH_REQUIRED', 'Sign in to generate resources.');
  // Public project API key: ties Google's token lookup to TEACHR's Firebase project.
  const response = UrlFetchApp.fetch('https://identitytoolkit.googleapis.com/v1/accounts:lookup?key=AIzaSyAF4zqPPPqoYA1hwNyorfqinsQkglNp6lQ', {
    method: 'post', contentType: 'application/json', payload: JSON.stringify({ idToken }), muteHttpExceptions: true
  });
  if (response.getResponseCode() !== 200) throw generationError_(401, 'INVALID_TOKEN', 'Your sign-in session is invalid or expired.');
  const user = JSON.parse(response.getContentText()).users?.[0];
  if (!user || !user.localId || user.disabled) throw generationError_(401, 'INVALID_TOKEN', 'Your sign-in session is invalid or expired.');
  if (!user.emailVerified) throw generationError_(403, 'EMAIL_NOT_VERIFIED', 'Verify your email address before generating resources.');
  return user.localId;
}
function generationFirestore_(suffix, method, body, missingAllowed) {
  ...
  const response = UrlFetchApp.fetch('https://firestore.googleapis.com/v1/projects/' + CONFIG.FIREBASE_PROJECT_ID + '/databases/(default)/documents' + suffix, options);
  const status = response.getResponseCode();
  if (status === 404 && missingAllowed) return null;
  if (status >= 300) {
    const error = generationError_(503, 'USAGE_UNAVAILABLE', 'Generation usage could not be verified.');
    error.retryable = status === 409; throw error;
  }
  return JSON.parse(response.getContentText() || '{}');
}
function generationFields_(doc) {
  const result = {};
...
function generationAccess_(uid, tool, transaction) {
  ...
  const remaining = Math.max(0, record.allowance - record.successfulGenerations);
  if (!unlimited && remaining === 0) throw generationError_(429, 'FREE_LIMIT_REACHED', 'Your free generations for this tool are used. Upgrade to continue.');
  return { unlimited, remaining: unlimited ? null : remaining, record };
}
function recordGenerationSuccess_(uid, tool) {
  // A transaction re-reads role and allowance after generation. Concurrent calls,
  // subscription changes and admin adjustments cannot overwrite each other.
  for (let attempt = 0; attempt < 3; attempt++) {
    ...
    let committed = false;
    try {
      const access = generationAccess_(uid, tool, transaction);
      const writes = access.unlimited ? [] : [{
        update: {
          name: 'projects/' + CONFIG.FIREBASE_PROJECT_ID + '/databases/(default)/documents/users/' + uid + '/usage/' + tool,
          fields: { toolId: { stringValue: tool }, successfulGenerations: { integerValue: String(access.record.successfulGenerations + 1) }, allowance: { integerValue: String(access.record.allowance) } }
        },
        updateMask: { fieldPaths: ['toolId', 'successfulGenerations', 'allowance'] },
        updateTransforms: [{ fieldPath: 'updatedAt', setToServerValue: 'REQUEST_TIME' }]
      }];
      generationFirestore_(':commit', 'post', { transaction, writes });
      committed = true;
      return { unlimited: access.unlimited, remaining: access.unlimited ? null : access.remaining - 1 };
    } catch (error) {
      if (!error.retryable || attempt === 2) throw error;
    } finally {
      if (!committed) { try { generationFirestore_(':rollback', 'post', { transaction }); } catch (_) { /* Best effort transaction cleanup. */ } }
    }
  }
}
function generateResource_(body) {
  ...
  try {
    ...
    if (response.getResponseCode() !== 200) throw new Error('Provider failure');
